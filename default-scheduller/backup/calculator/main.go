...
func main() {
	opt := &redis.Options{
		Addr:     "54.255.223.141:6379",
		Username: "",
		Password: "Vananh12345",
		DB:       0,
	}

	rdb = redis.NewClient(opt)

	connStr := "postgres://postgres:Vananh12345@54.255.223.141:5432/postgres?sslmode=disable"
	...
	// Use Ping() to verify the connection is actually established
	err = db.Ping()
	if err != nil {
		panic(err)
	}
	...
	fmt.Println("Ping redis:", val) // >>> foo bar

	for {
		start := time.Now()
		var data_Metrics []MetricData
		rows, err := db.Query("select * from MetricData")
		if err != nil {
			panic(err)
		}
		defer rows.Close()
		...
		GobalMin_CPUUsage = data_Metrics[0].CPUUsageMin
		GobalMin_MemoryUsage = data_Metrics[0].MemoryUsageMin
		GobalMin_TailLatency = data_Metrics[0].TailLatencyMin
		GobalMax_DiskFree = data_Metrics[0].DiskFreeMax
		GobalMin_SwapMemory = data_Metrics[0].SwapMemory_max
		GobalMin_DiskIOCounterBusyTime = data_Metrics[0].DiskIOCounterBusyTimeMin
		GobalMin_NetIOCounterDropin = data_Metrics[0].NetIOCounterDropinMin
		GobalMin_NetIOCounterDropout = data_Metrics[0].NetIOCounterDropoutMin
